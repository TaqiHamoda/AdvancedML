...
class NormalizeTransform(torch.nn.Module):
    def __init__(self):
        ...
        self.transform = v2.Compose([
            ...
            v2.Normalize(mean=[0.77146477], std=[0.13823779])
        ])
...
class SonarDataset(Dataset):
    ...
    def __init__(self, data_dir="data/processed", ext="*.npz"):
        ...
        self.h_flip = v2.RandomHorizontalFlip(p=1.0)
        self.v_flip = v2.RandomVerticalFlip(p=1.0)
    ...
    def __getitem__(self, idx):
        ...
        if torch.rand(1) >= 0.5:
            data = self.h_flip(data)
            distances = self.h_flip(distances)

        if torch.rand(1) >= 0.5:
            data = self.v_flip(data)
            distances = self.v_flip(distances)
