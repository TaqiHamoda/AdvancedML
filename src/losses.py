...
class SinkhornKnopp(nn.Module):
    ...
    def __init__(self, out_dim, student_temp=0.1, center_momentum=0.996, n_iterations=3, eps=1e-6):
        ...
        camera configurations, and locations/subjects. On the other hand, sonar is a monochromatic modality that
        often has repetitive textures and backgrounds with very little variation. Also, the dataset we are using
        has only 1 million tiles all from the same sensor, configuration, and location. Given that, a prototype
        count that is 32 times smaller than DINOv3 felt appropriate. After all, the prototype count should be small
        enough to force a bottleneck to motivate the model to learn, but large enough to allow the model to express
        a wide-variety of rich, dense, and representative semantic features.
        """
        super().__init__()
